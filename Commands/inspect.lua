        ...
        if arg then
            ...
            if if_have == true then
                ...
                if inspectid == nil then message.channel:send(message.author.username..", did you input the id correctly? ``ex: h$inspect 94``") --if no numbers
                    ...
                    else if inspectid > 256 then message.channel:send(message.author.username..", this artifact does not exist!") -- if id is bigger than existing ids
                end end end
            end
        else message.channel:send(message.author.username..", please input an artifacts id number!") --if no args
